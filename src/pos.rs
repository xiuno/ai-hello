...
pub mod rope {
    ...
    mod tests {
        ...
        fn test_rope_encoding() -> Result<()> {
            ...
            // 应用 RoPE
            let result = rope::encoding(&x, 0, Some(10000.0))?;
        ...
        fn test_rope_with_cache() -> Result<()> {
            ...
            let (cos_cache, sin_cache) =
                rope::precompute_rope_cache(100, 8, 10000.0, &device, dtype)?;
            ...
            // 使用缓存应用 RoPE
            let result = rope::encoding_with_cache(&x, 0, &cos_cache, &sin_cache)?;
        ...
        fn test_frequency_creation() -> Result<()> {
            ...
            let freqs = rope::create_frequencies_simple(8, 10000.0, &device, dtype)?;
