...
struct NeuralNetwork {
    ...
    bn2: BatchNorm,
    // 池化  [B, 64, 14, 14]
    conv3: Conv2d, // 输入: [B, 64, 14, 14] -> 输出: [B, 64, 7, 7]
    // 池化  [B, 64, 7, 7]
    bn3: BatchNorm,
    dropout1: Dropout,
    dropout2: Dropout,
...
impl NeuralNetwork {
    fn new(vs: VarBuilder) -> Result<Self> {
        ...
        let conv3 = candle_nn::conv2d(64, 64, 3, conv_cfg, vs.pp("conv3"))?; //
        let bn3 = candle_nn::batch_norm(64, 1e-5, vs.pp("bn3"))?;
        ...
        Ok(Self { conv1, bn1, conv2, bn2, conv3, bn3, dropout1, dropout2, fc1, fc2 })
    }
    fn forward(&self, input: &Tensor, train: bool) -> Result<Tensor> {
        let x = self.conv1.forward(&input)?;
        let x = self.bn1.forward_t(&x, train)?.silu()?;
        let x = self.conv2.forward(&x)?;
        let x = self.bn2.forward_t(&x, train)?.silu()?;
        let x = x.max_pool2d(2)?;
        let x = self.conv3.forward(&x)?;
        let x = self.bn3.forward_t(&x, train)?.silu()?;
        let x = x.max_pool2d(2)?;
        ...
        // 输出层, 这里不能使用 relu() !
        //let x = if train { self.dropout2.forward(&x, train)? } else { x };
...
fn main() -> Result<()> {
    ...
    println!("开始训练...");
    const BATCH_SIZE: usize = 64; // 每批处理的图片数
    const CHUNKS: usize = 60000 / BATCH_SIZE;  // ≈ 937 次更新

    let image_batches = train_images.chunk(CHUNKS, 0)?; // 60000/CHUNKS
    let label_batches = train_labels.chunk(CHUNKS, 0)?; // 60000/CHUNKS
    for epoch in 0..10 {
        ...
        // todo: 优化, 此段代码可删除
        if epoch > 3 && accuracy < 97.0 {
            let new_lr = optimizer.learning_rate() * 0.95;
