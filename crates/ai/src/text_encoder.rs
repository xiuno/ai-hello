...
impl TextEncoder {
    pub fn new(model_dir: impl AsRef<Path>) -> Result<TextEncoder> {
        ...
        // 2. 加载 tokenizer
        let model_dir = model_dir.as_ref(); // 转为 &Path，不克隆
    ...
    pub fn encode<S: AsRef<str>>(&self, sentences: Vec<S>) -> Result<Vec<Vec<f32>>> {
        // 5. 编码并获取 embeddings
        let mut ret = vec![];
...
fn test_1() {
    ...
    let textencoder = TextEncoder::new("../../models/minilm").unwrap();
